...
app.post("/StaffSignIn", (req, res) => {
    let input = req.body
    StaffModel.find({ "Staff EmailId": req.body.StaffEmailId }).then(
        (response) => {
            if (response.length > 0) {
                let dbPassword = response[0].StaffPassword
                console.log(dbPassword)
                bcryptjs.compare(input.StaffPassword, dbPassword, (error, isMatch) => {
                    if (isMatch) {
                        jwt.sign({ StaffEmailId: input.StaffEmailId }, "mess-app", { expiresIn: "1d" },
                            (error, token) => {
                                if (error) {
                                    res.json({ "status": "unable to create token" })
                                } else {
